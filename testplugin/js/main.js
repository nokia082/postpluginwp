
// This is the client-side script.

// Initialize the HTTP request.


// quick add post ajax

var quickAddButton = document.querySelector("#quick-add-button");
var portfolioPostsContainer1 = document.getElementById("portfolio-posts-container1");
var portfolioPostsContainer2 = document.getElementById("portfolio-posts-container2");

	quickAddButton.addEventListener("click", function() 
 {
    one();
    two();
 });

function one()
{



	
		var ourPostData = 
		{
			"title": document.querySelector('.admin-quick-add [name="title"]').value,
			"content": document.querySelector('.admin-quick-add [name="content"]').value,
			"status": "publish"
		}
		var createPost = new XMLHttpRequest();
		createPost.open('POST', 'http://localhost/wordpress/wp-json/wp/v2/posts');
		createPost.setRequestHeader("X-WP-Nonce", magicalData.nonce);
		createPost.setRequestHeader("Content-Type","application/json;charset=UTF-8");
		createPost.send(JSON.stringify(ourPostData));
		createPost.onreadystatechange = function() 
		{
			if(createPost.readyState == 4) 
			{
				if(createPost.status == 201)
				{
document.querySelector('.admin-quick-add [name="title"]').value ='';
document.querySelector('.admin-quick-add [name="content"]').value = '';
alert("Post added");
				}
				else 
				{
					  console.log(data);
				}
			}
		}

};
//posts display


function two()
{

var ourRequest = new XMLHttpRequest();
ourRequest.open('GET', 'http://localhost/wordpress/wp-json/wp/v2/posts');
	ourRequest.onload = function() 
	{
  		if (ourRequest.status >= 200 && ourRequest.status < 400) 
 	 	{
   	 	var data = JSON.parse(ourRequest.responseText);
   	 	createHTML1(data);
   	 	createHTML2(data);
  		} 
  		else 
  		{
    	console.log("We connected to the server, but it returned an error.");
  		}

	};

	ourRequest.onerror = function() 
		{
  		console.log("Connection error");
		};

		ourRequest.send();
}; 


function createHTML1(postsData)
{
	var ourHTMLString = '';
	for (i=0; i<=0; i++)
	{
	ourHTMLString += '<h1><b>' + postsData[i].title.rendered + '</b></h1>';
	ourHTMLString += '<b><h3>' + postsData[i].content.rendered + '</b></h3>';
	}
portfolioPostsContainer1.innerHTML = ourHTMLString;
}



function createHTML2(postsData)
{
	var ourHTMLString = '';
	for (i=1; i < postsData.length; i++)
	{
	ourHTMLString += '<h3><b>' + postsData[i].title.rendered + '</b></h3>';
	ourHTMLString += '<b>' + postsData[i].content.rendered + '</b>';
	}
portfolioPostsContainer2.innerHTML = ourHTMLString;
}

